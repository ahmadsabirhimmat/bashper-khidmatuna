...
export const GoogleSignInButton = ({ busy, disabled, label, waitingLabel, onClick }) => (
  <button
    type="button"
    disabled={disabled || busy}
    onClick={onClick}
    className="mt-4 flex w-full items-center justify-center gap-3 rounded-full border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-800 shadow-sm transition hover:border-blue-300 hover:bg-slate-50 disabled:opacity-60"
  >
    {busy ? (
      waitingLabel
    ) : (
